...
// frequency limits
#define DEFAULT_C_STATE		300000
#define DEFAULT_HI_STATE	787200
#define DEFAULT_MID_STATE	1190400
#define DEFAULT_LOW_STATE	1410600

// temperature limits
#define DEFAULT_C_LEVEL		12
#define DEFAULT_HI_LEVEL	9
#define DEFAULT_MID_LEVEL	5

/********** sysfs start **********/
unsigned int temp_threshold = DEFAULT_TEMP_LIMIT;
module_param(temp_threshold, int, 0755);

unsigned int safe_diff = DEFAULT_SAFE_DIFF;
module_param(safe_diff, int, 0755);

unsigned int c_state = DEFAULT_C_STATE;
module_param(c_state, int, 0755);

unsigned int hi_state = DEFAULT_HI_STATE;
module_param(hi_state, int, 0755);

unsigned int mid_state = DEFAULT_MID_STATE;
module_param(mid_state, int, 0755);

unsigned int low_state = DEFAULT_LOW_STATE;
module_param(low_state, int, 0755);

unsigned int c_level = DEFAULT_C_LEVEL;
module_param(c_level, int, 0755);

unsigned int hi_level = DEFAULT_HI_LEVEL;
module_param(hi_level, int, 0755);

unsigned int mid_level = DEFAULT_MID_LEVEL;
module_param(mid_level, int, 0755);
/********** sysfs end **********/
...
static void check_temp(struct work_struct *work)
{
	...
	if (info.throttling) {
		if (temp < (temp_threshold - safe_diff)) {
	...
	if (temp >= temp_threshold + c_level)
		freq = c_state;
	else if (temp >= temp_threshold + hi_level)
		freq = hi_state;
	else if (temp >= temp_threshold + mid_level)
		freq = mid_state;
