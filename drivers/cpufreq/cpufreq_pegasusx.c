...
/* cpufreq_pegasusx Governor Tunables */
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("pegasusx_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create pegasusx_dbs_wq workqueue\n");
		return -EFAULT;
	}

	ret = cpufreq_register_governor(&cpufreq_gov_pegasusx);
	if (ret)
		goto err_reg;
...
{
	cpufreq_unregister_governor(&cpufreq_gov_pegasusx);
...
MODULE_DESCRIPTION("'cpufreq_pegasusx' - A dynamic cpufreq governor");
MODULE_LICENSE("GPLv2");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PEGASUSX
